...
const Curtain = () => {
  ...
  const fetchCurtainDevice = async () => {
    const response = await fetch(
      "http://localhost:8080/devices/645c48d6781e46b6e35c4fe6"
    );
    const responseData = await response.json();

    setCurtainStatus(responseData.status === "on" ? true : false);
  };
  ...
  const handleSwitchToggle = async (value) => {
    ...
    const response = await fetch(
      "http://localhost:8080/devices/645c48d6781e46b6e35c4fe6/status",
      {
        method: "PUT",
        body: value ? "on" : "off",
      }
